...
void dm3519_current_set(hcan_t* hcan, uint16_t id, float m1_cur_set, float m2_cur_set, float m3_cur_set, float m4_cur_set)
{
	...
	int16_t m1_cur_tmp = m1_cur_set*(16384.0f/20.0f);
	int16_t m2_cur_tmp = m2_cur_set*(16384.0f/20.0f);
	int16_t m3_cur_tmp = m3_cur_set*(16384.0f/20.0f);
	int16_t m4_cur_tmp = m4_cur_set*(16384.0f/20.0f);
...
void dm3519_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	motor->para.pos =  pos_int16/8192.0f*360.0f;
	motor->para.vel =  (float)spd_int16;
...
void dm_motor_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	motor->para.pos = uint_to_float(motor->para.p_int, 0, motor->tmp.PMAX, 16); // (-12.5,12.5)
...
************************************************************************
**/
void ead_motor_ctrl_fbdata(uint16_t id) 
{
	uint8_t can_id_l = id & 0xFF;       // 低 8 位
    uint8_t can_id_h = (id >> 8) & 0x07; // 高 3 位
